...
be used in Aurum Core with the `-asmap` option.
...
to binary before being used with Aurum Core.
...
the `aurum-cli getnodeaddresses` command.
...
```
aurum-cli getnodeaddresses 0 > addrs.json
```
