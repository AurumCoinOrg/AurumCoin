# CJDNS support in Aurum Core

It is possible to run Aurum Core over CJDNS, an encrypted IPv6 network that
...
done outside of Aurum Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Aurum network and individual nodes.
...
before it will work with your Aurum Core node. This is described in steps
...
## Run Aurum Core with CJDNS

Once you are connected to the CJDNS network, the following Aurum Core
...
When enabled, this option tells Aurum Core that it is running in an
...
IPv6 local network. This helps Aurum Core perform better address management:
...
CJDNS support was added to Aurum Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `aurum-cli -addrinfo` to see the
...
## CJDNS-related information in Aurum Core

There are several ways to see your CJDNS address in Aurum Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `aurum-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `aurum-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `aurum-cli help getnodeaddresses` for details.
