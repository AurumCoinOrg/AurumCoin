...
It is possible to run Aurum Core as a Tor onion service, and connect to such services.
...
- Starting with version 22.0, Aurum Core only supports Tor version 3 hidden
  services (Tor v3). Tor v2 addresses are ignored by Aurum Core and neither
  relayed nor stored.

- Tor removed v2 support beginning with version 0.4.6.

## How to see information about your Tor configuration via Aurum Core

There are several ways to see your local onion address in Aurum Core:
...
You can use the `getnodeaddresses` RPC to fetch a number of onion peers known to your node; run `aurum-cli help getnodeaddresses` for details.

`bitcoin rpc` can also be substituted for `aurum-cli`.

## 1. Run Aurum Core behind a Tor proxy

The first step is running Aurum Core behind a Tor proxy. This will already anonymize all
...
In a typical situation, this suffices to run behind a Tor proxy:
    ...
    aurumd -proxy=127.0.0.1:9050

`bitcoin node` or `bitcoin gui` can also be substituted for `aurumd`.

## 2. Automatically create a Aurum Core onion service

Aurum Core makes use of Tor's control socket API to create and destroy
ephemeral onion services programmatically. This means that if Tor is running and
proper authentication has been configured, Aurum Core automatically creates an
onion service to listen on. The goal is to increase the number of available
onion nodes.

This feature is enabled by default if Aurum Core is listening (`-listen`) and
it requires a Tor connection to work. It can be explicitly disabled with
`-listenonion=0`. If it is not disabled, it can be configured using the
`-torcontrol` and `-torpassword` settings.

To see verbose Tor information in the aurumd debug log, pass `-debug=tor`.
...
methods to be configured: cookie authentication or aurumd's `-torpassword`
configuration option.

#### Cookie authentication

For cookie authentication, the user running aurumd must have read access to
the `CookieAuthFile` specified in the Tor configuration. In some cases this is
preconfigured and the creation of an onion service is automatic. Don't forget to
use the `-debug=tor` aurumd configuration option to enable Tor debug logging.

If a permissions problem is seen in the debug log, e.g. `tor: Authentication
cookie /run/tor/control.authcookie could not be opened (check permissions)`, it
can be resolved by adding both the user running Tor and the user running
aurumd to the same Tor group and setting permissions appropriately.
...
## 3. Manually create a Aurum Core onion service
...
your aurumd's P2P listen port (8333 by default), and target addresses and ports
...
In a typical situation, where you're only reachable via Tor, this should suffice:
    ...
    aurumd -proxy=127.0.0.1:9050 -externalip=7zvj7a2imdgkdbg4f2dryd5rgtrn7upivr5eeij4cicjh65pooxeshid.onion -listen

(obviously, replace the .onion address with your own). It should be noted that you still
listen on all devices and another node could establish a clearnet connection, when knowing
your address. To mitigate this, additionally bind the address of your Tor proxy:

    aurumd ... -bind=127.0.0.1:8334=onion
...
for normal IPv4/IPv6 communication, use:
    ...
    aurumd -onion=127.0.0.1:9050 -externalip=7zvj7a2imdgkdbg4f2dryd5rgtrn7upivr5eeij4cicjh65pooxeshid.onion -discover

## 4. Privacy recommendations

- Do not add anything but Aurum Core ports to the onion service created in section 3.
  If you run a web service too, create a new onion service for that.
  Otherwise it is trivial to link them, which may reduce privacy. Onion
  services created automatically (as in section 2) always have only one port
  open.
- Operating a node that listens on multiple networks (e.g. IPv4 and Tor) can help
  strengthen the Aurum network, as nodes in this configuration (i.e. bridge nodes) increase
