...
Aurum Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
configuration options and toggled while aurumd is running with the `logging`
RPC.  For instance, launching aurumd with `-debug` or `-debug=1` will turn on
...
Aurum Core is a multi-threaded application, and deadlocks or other
...
You can then use the `-debug=lock` configuration option at aurumd startup or
`aurum-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `aurum-cli logging [] '["lock"]'`.
...
$ valgrind -v --leak-check=full build/bin/aurumd -printtoconsole
...
llvm-cov show \
    ...
    --project-title="Aurum Core Coverage Report"
```
...
llvm-cov show \
    ...
    --show-expansions \
    --output-dir=build/coverage_report \
    --project-title="Aurum Core Fuzz Coverage Report"
```
...
To profile a running aurumd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep aurumd` -- sleep 60
```
...
Aurum Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`aurumd`)](https://doxygen.bitcoincore.org/bitcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
  : Started from `main()` in `aurumd.cpp`. Responsible for starting up and
...
pay attention to for reviewers of Aurum Core code.
...
- For `strprintf`, `LogInfo`, `LogDebug`, etc formatting characters don't need size specifiers (hh, h, l, ll, j, z, t, L) for arithmetic types.
  ...
  - *Rationale*: Aurum Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Normally, these are maintained by active developers of Aurum Core, in which case
...
a Aurum Core subtree, so that they can be integrated quickly. Cosmetic changes
...
The tool instructions also include a list of the subtrees managed by Aurum Core.
...
- src/leveldb
  - Upstream at https://github.com/google/leveldb ; maintained by Google. Open
    important PRs to the Aurum Core subtree to avoid delay.
...
file descriptors, it will cause problems with Aurum's `select()` loop, because
...
compatibility between nodes. This happened in Aurum 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Aurum's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `aurum-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `aurum-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Aurum Core, the wallet was always
