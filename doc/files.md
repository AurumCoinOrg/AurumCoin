...
The data directory is the default location where the Aurum Core files are stored.
...
---------|--------------------
Linux    | `$HOME/.bitcoin/`
macOS    | `$HOME/Library/Application Support/Aurum/`
Windows  | `%LOCALAPPDATA%\Aurum\` <sup>[\[1\]](#note1)</sup>
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Aurum blocks (dumped in network format, 128 MiB per file)
...
`./`               | `bitcoin.conf`        | User-defined [configuration settings](bitcoin-conf.md) for `aurumd` or `bitcoin-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `aurumd.pid`        | Stores the process ID (PID) of `aurumd` or `bitcoin-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `aurumd` or `bitcoin-qt`; can be specified by `-debuglogfile` option
...
These subdirectories and files are no longer used by Aurum Core:
...
This table describes the files installed by Aurum Core across different platforms.
...
| bin/bitcoin                                                | Command-line tool for interacting with Aurum. Calls other binaries below. |
| bin/aurum-cli                                            | Tool for making node and wallet RPC calls.                                  |
| bin/bitcoin-qt                                             | Aurum node and wallet GUI                                                 |
| bin/bitcoin-tx                                             | Tool for creating and modifying transactions                                |
| bin/bitcoin-util                                           | Miscellaneous utilities                                                     |
| bin/aurum-wallet                                         | Aurum wallet tool                                                         |
| bin/aurumd                                               | Aurum node and wallet daemon                                              |
...
| libexec/bitcoin-node                                       | IPC-enabled alternative to `aurumd`                                       |
| libexec/test_bitcoin                                       | Unit test binary                                                            |
| *libexec/test_bitcoin-qt*                                  | GUI-specific unit tests                                                     |
| share/man/man1/                                            | Man pages for command-line tools like `aurum-cli`, `aurumd`, and others |
...
- On Windows, binaries have a `.exe` suffix (e.g., `aurum-cli.exe`).
...
There have been multiple reports of database corruption and data loss when using this filesystem with Aurum Core, see [Issue #31454](https://github.com/bitcoin/bitcoin/issues/31454) for more details.
