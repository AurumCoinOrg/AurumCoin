Aurum Core version 28.0 is now available from:
...
installer (on Windows) or just copy over `/Applications/Aurum-Qt` (on macOS)
...
Running Aurum Core binaries on macOS requires self signing.
```
cd /path/to/bitcoin-28.0/bin
xattr -d com.apple.quarantine aurum-cli bitcoin-qt bitcoin-tx bitcoin-util aurum-wallet aurumd test_bitcoin
codesign -s - aurum-cli bitcoin-qt bitcoin-tx bitcoin-util aurum-wallet aurumd test_bitcoin
```
...
using the Linux Kernel 3.17+, macOS 11.0+, and Windows 7 and newer. Aurum
...
The default data directory on Windows has been moved from `C:\Users\Username\AppData\Roaming\Aurum`
to `C:\Users\Username\AppData\Local\Aurum`. Aurum Core will check the existence
...
- Previously if Aurum Core was listening for P2P connections, either using
...
- Aurum Core will now fail to start up if any of its P2P binds fail, rather
...
- The `warnings` field in `getblockchaininfo`, `getmininginfo` and
  ...
  can be temporarily restored by running Aurum Core with the configuration
...
are now required to compile Aurum Core. (#29091, #30263)

- The minimum required glibc to run Aurum Core is now
...
Previous releases of Aurum Core or previous external software will not be able to read the blocksdir with a non-zero XOR-key.
