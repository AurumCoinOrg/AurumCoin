...
Aurum Core version 26.0 is now available from:
...
- `aurumd` and `bitcoin-qt` will now raise an error on startup
...
- Wallet loading has changed in this release. Wallets with some corrupted records that could be
  ...
  load the wallet in a previous version of Aurum Core and import the data into a new wallet.
...
```sh
src/aurum-cli -named bumpfee txid fee_rate=100
```

instead of

```sh
src/aurum-cli -named bumpfee txid options='{"fee_rate": 100}'
```
...
- The usage of hybrid public keys in output descriptors has been removed. Hybrid
  ...
  (as specified in BIP380 and documented in doc/descriptors.md). Aurum Core would
