Aurum Core version 0.12.0 is now available from:
...
installer (on Windows) or just copy over /Applications/Aurum-Qt (on Mac) or
aurumd/bitcoin-qt (on Linux).
...
backwards-compatible with pre-0.10 versions of Aurum Core or other software:
...
pre-0.12 versions of Aurum Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Aurum Core version 0.11 or
earlier.
...
ECDSA signatures inside Aurum transactions now use validation using
...
Previous versions of Aurum Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Aurum Core was to raise the default minimum
relay fee.

Aurum Core 0.12 will have a strict maximum size on the mempool. The
...
Aurum Core 0.12 also introduces new default policy limits on the length and
...
Aurum Core 0.12 nodes. Aurum Core will only allow replacement of
...
Note that the wallet in Aurum Core 0.12 does not yet have support for
...
This allows running aurumd without having to do any manual configuration.
...
Aurum Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Aurum Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Aurum Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Aurum Core 0.13, and it is at this time undecided whether the
...
Aurum Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Aurum Core automatically creates a hidden service to listen on, without
manual configuration. Aurum Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Aurum Core is listening, and
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Aurum
Core to use floating fees. By default, Aurum Core will use floating
fees.
...
At all times, Aurum Core will cap fees at `-maxtxfee=<x>` (default:
0.10) AUR.
Furthermore, Aurum Core will never create transactions paying less than
...
caching. A sample config for apache2 could look like:
    ...
    <Location /bitcoinrpc>
        ...
        # optional bypass aurumd rpc basic auth
...
Restarting aurumd will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of Aurum-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
...
- #6730 `cb4d6d0` build: Remove dependency of aurum-cli on secp256k1 (Wladimir J. van der Laan)
...
- #6387 `11576a5` [aurum-cli] improve error output (Jonas Schnelli)
