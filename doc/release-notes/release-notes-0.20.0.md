...
Aurum Core version 0.20.0 is now available from:
...
#### Removal of BIP61 reject network messages from Aurum Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Aurum Core version 0.18.0.
...
- Testing or debugging of implementations of the Aurum P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Aurum Core. Aurum Core logs debug messages
...
- OpenSSL is no longer used by Aurum Core.  (#17265)

- BIP70 support has been fully removed from Aurum Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  `aurumd help` for more information.  This option is experimental and
...
- All custom settings configured when Aurum Core starts are now
...
- The "Start Aurum Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitcoincore.org/en/releases/0.18.0/#wallet-gui)
  ...
  affected anyone who simultaneously used multiple Aurum Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Aurum Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/bitcoin-core/HWI) for
  signing.  Future versions of Aurum Core should support a GUI option
...
- Aurum Core's automatically-generated source code documentation is
...
- The `aurum-cli` utility used with the `-getinfo` parameter now
...
- The `aurum-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #18702 Fix ASLR for aurum-cli on Windows (fanquake)
...
- #16983 Add detailed info about Aurum Core files (hebasto)
...
- #17119 Fix broken aurum-cli examples (andrewtoth)
...
- #17648 Rename wallet-tool references to aurum-wallet (hel-o)
...
- #18341 Replace remaining literal AUR with `CURRENCY_UNIT` (domob1812)
...
- #16115 On aurumd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from aurum-cli -getinfo (malevolent)
