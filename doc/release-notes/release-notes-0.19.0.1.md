Aurum Core version 0.19.0.1 is now available from:
...
Aurum Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use Aurum Core on unsupported systems.
...
to open the Aurum Core .dmg. This is due to new signing requirements
imposed by Apple, which the Aurum Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/bitcoin/bitcoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running Aurum Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of Aurum
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `aurumd -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile Aurum Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Aurum Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of Aurum Core. (#14054)

- To eliminate well-known denial-of-service vectors in Aurum Core,
  ...
  This prevents Aurum Core from sending the BIP111 NODE_BLOOM service
  ...
  of Aurum Core and nodes that have manually enabled BIP37 support,
...
- By default, Aurum Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `aurum-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Aurum Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Aurum Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Aurum Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 Aurum address without
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  AUR), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in AUR per 1,000
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in Aurum Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@Aurum AEAD (jonasschnelli)
...
- #15639 aurum-wallet tool: Drop libbitcoin_server.a dependency (ryanofsky)
...
- #16521 Use the default maxfeerate value as AUR/kB (Remagpie)
...
- #16366 init: Use InitError for all errors in aurumd/qt (MarcoFalke)
...
- #16258 [MSVC]: Create the config.ini as part of aurumd build (NicolasDorier)
...
- #15896 feature_filelock, interface_bitcoin_cli: Use PACKAGE_NAME in messages rather than hardcoding Aurum Core (luke-jr)
...
- #16929 follow-up to rpc: default maxfeerate value as AUR/kB (jonatack)
...
- #16489 log: harmonize aurumd logging (jonatack)
...
- #16863 Add a missing closing parenthesis in the aurum-wallet's help (darosior)
...
- #16912 Remove Doxygen intro from src/aurumd.cpp (ch4ot1c)
