Aurum version 0.5.0 is now available for download at:
http://sourceforge.net/projects/bitcoin/files/Aurum/bitcoin-0.5.0/
...
The wallet encryption feature introduced in Aurum version 0.4.0 did not sufficiently secure the private keys. An attacker who
managed to get a copy of your encrypted wallet.dat file might be able to recover some or all of the unencrypted keys and steal the
associated coins.

If you have a previously encrypted wallet.dat, the first time you run bitcoin-qt or aurumd the wallet will be rewritten, Aurum will
...
Wallets encrypted with this version of Aurum are written properly.
...
1. Run Aurum and let it rewrite the wallet.dat file

2. Run it again, then ask it for a new bitcoin address.
Aurum-Qt: Address Book, then New Address...
aurumd: run the 'walletpassphrase' RPC command to unlock the wallet,  then run the 'getnewaddress' RPC command.

3. If your encrypted wallet.dat may have been copied or stolen, send  all of your bitcoins to the new bitcoin address.

4. Shut down Aurum, then backup the wallet.dat file.
