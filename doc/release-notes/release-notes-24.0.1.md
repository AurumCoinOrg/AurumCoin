...
Aurum Core version 24.0.1 is now available from:
...
using the Linux kernel, macOS 10.15+, and Windows 7 and newer.  Aurum
...
This version of Aurum Core adds a new `mempoolfullrbf` configuration
...
Some Aurum services today expect that the first version of an
...
The Aurum Protocol does not, and cannot, provide any assurance that
...
fees, was realized in [Aurum Core 0.12.0][] (February 2016) with the
...
default for years, and several contributors to Aurum Core are
...
[Aurum Core 0.12.0]: https://bitcoincore.org/en/releases/0.12.0/#opt-in-replace-by-fee-transactions
...
- To help prevent fingerprinting transactions created by the Aurum Core wallet, change output
...
config files), so these settings will now apply to aurumd, instead of being
