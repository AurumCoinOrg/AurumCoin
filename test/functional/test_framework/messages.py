...
# Copyright (c) 2010-present The Aurum Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
"""Aurum test framework primitive and message structures
...
# Objects that map to aurumd objects, which can be serialized/deserialized
...
class CBlockLocator:
    ...
    def serialize(self):
        r = b""
        r += (0).to_bytes(4, "little", signed=True)  # Aurum Core ignores the version field. Set it to 0.
...
class CTransaction:
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in aurumd
            if (flags != 0):
                self.vin = deser_vector(f, CTxIn)
...
class msg_version:
    ...
    def deserialize(self, f):
        ...
        # But, unconditionally check it to match behaviour in aurumd
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in aurumd indicates these should be deserialized as blocks
