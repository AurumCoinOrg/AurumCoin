...
"""Class for aurumd node under test"""
...
BITCOIN_PID_FILENAME_DEFAULT = "aurumd.pid"
...
class TestNode():
    """A class for representing a aurumd node under test.
    ...
    def __init__(self, i, datadir_path, *, chain, rpchost, timewait, timeout_factor, binaries, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, v2transport=False, uses_wallet=False, ipcbind=False):
        ...
        # This means that starting a aurumd using the temp dir to debug a failed test won't
        # spam debug.log.
    ...
    def __del__(self):
        # Ensure that we don't leave any aurumd processes lying around after
        # the test ends
        if self.process:
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        # If listening and no -bind is given, then aurumd would bind P2P ports on
        ...
        # Add a new stdout and stderr file each time aurumd is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by aurumd, and
        ...
        self.running = True
        self.log.debug("aurumd started, waiting for RPC to come up")

        if self.start_perf:
            self._start_perf()

    def wait_for_rpc_connection(self, *, wait_for_import=True):
        """Sets up an RPC connection to the aurumd process. Returns False if unable to connect."""
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                ...
                raise FailedToStartError(self._node_msg(
                    f'aurumd exited with status {self.process.returncode} during initialization. {str_error}'))
            try:
                rpc = get_rpc_proxy(
                ...
                # Suppress if cookie file isn't generated yet and no rpcuser or rpcpassword; aurumd may be starting.
                if "No RPC credentials" not in str(e):
                    raise
                latest_error = suppress_error("missing_credentials", e)
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error(f"Unable to connect to aurumd after {self.rpc_timeout}s (ignored errors: {dict(suppressed_errors)!s}{'' if latest_error is None else f', latest: {latest_error[0]!r}/{latest_error[1]}'})")
    ...
    def wait_for_cookie_credentials(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            try:
                ...
                return
            except ValueError:  # cookie file not found and no rpcuser or rpcpassword; aurumd is still starting
    ...
    def _start_perf(self, profile_name=None):
        ...
        if not test_success('readelf -S {} | grep .debug_str'.format(shlex.quote(self.binary))):
            self.log.warning(
                "perf output won't be very useful without debug symbols compiled into aurumd")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to aurumd
        expected_msg: regex that stderr should match when aurumd fails

        Will raise if aurumd starts without an error.
        Will raise if an expected_msg is provided and it does not match aurumd's stdout."""
        ...
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug(self._node_msg(f'aurumd exited with status {ret} during initialization'))
                ...
                self.running = False
                self.process = None
                assert_msg = f'aurumd should have exited within {self.rpc_timeout}s '
...
class TestNodeCLI():
    """Interface to aurum-cli for an individual node"""
    ...
    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with aurum-cli command-line options
    ...
    def send_cli(self, clicommand=None, *args, **kwargs):
        """Run aurum-cli command. Deserializes returned string as python object."""
        ...
        self.log.debug("Running aurum-cli {}".format(p_args[2:]))
