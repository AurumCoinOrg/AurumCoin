...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Aurum
    //   new: C:\Users\Username\AppData\Local\Aurum
    // macOS: ~/Library/Application Support/Aurum
    // Unix-like: ~/.bitcoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "AurumCoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "AurumCoin";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Aurum";
#else
    // Unix-like
