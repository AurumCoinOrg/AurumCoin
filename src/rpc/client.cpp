...
//! Specify whether parameter should be parsed by aurum-cli as a JSON value,
 ...
 *
 * JSON parameters need to be listed here to make aurum-cli parse command line
 * arguments as JSON, instead of passing them as raw strings. `JSON` and
 * `JSON_OR_STRING` formats both make `aurum-cli` attempt to parse the
 ...
 * `aurum-cli getblockstats <hash>` without needing to quote the hash string
 * as JSON (`'"<hash>"'`).
 *
 ...
 * to make aurum-cli treat them as positional parameters when `-named` is used.
 * This prevents `aurum-cli` from splitting strings like "my=wallet" into a named
 ...
 * other string parameters for that method need to be listed as well so aurum-cli
 ...
 *
 * For example, the command `aurum-cli -named createwallet "my=wallet"`,
